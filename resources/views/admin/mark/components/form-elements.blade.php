...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('exam_id'), 'has-success': this.fields.exam_id }">
    <label for="exam_id" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.exam.columns.exam_name') }}</label>
        ...
        <Multiselect v-model="form.exam_id" v-validate="'required'" :custom-label="examNames" :options="exams_data" :track-by="options.id" :searchable="true" :close-on-select="true" class="form-control" :class="{'form-control-danger': errors.has('exam_id'), 'form-control-success': this.fields.exam_id}" id="exam_id">
        </Multiselect >
