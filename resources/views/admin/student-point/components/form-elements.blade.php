...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('student_id'), 'has-success': this.fields.student_id }">
    <label for="student_id" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.student.columns.student_name') }}</label>
        ...
        <Multiselect v-model="form.student_id" v-validate="'required'" multiple :custom-label="nameWithAddress" :options="students_data" track-by="student_id" :searchable="true" :close-on-select="true" class="form-control" :class="{'form-control-danger': errors.has('student_id'), 'form-control-success': this.fields.student_id}" id="student_id">
        </Multiselect >
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('point_id'), 'has-success': this.fields.point_id }">
    <label for="point_id" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.point-config.columns.point_name') }}</label>
        ...
        <Multiselect v-model="form.point_id" v-validate="'required'" multiple :custom-label="nameWithpointConf" :options="points_data" track-by="id" :searchable="true" :close-on-select="true" class="form-control" :class="{'form-control-danger': errors.has('point_id'), 'form-control-success': this.fields.point_id}" id="point_id">
        </Multiselect >
